...
import cowsay
from mood.client.__main__ import CowNetcat

class TestClientCommands(unittest.TestCase):
    def setUp(self):
        self.socket_mock = MagicMock()
        self.client = CowNetcat(self.socket_mock)
    ...
    def test_attack_command_conversion(self):
        ...
        for inp, out in test_cases:
            ...
            self.client.s.sendall.assert_called_with(bytes(out.encode()))

    def test_attack_invalid_parameters(self):
        test_cases = [
            ("", "Specify monster name"),
            ("dragon with", "Attack with what?"),
            ("dragon with unknown_weapon", "Unknown weapon"),
            ("dragon spear", "Invalid arguments"),
            ("dragon with spear extra", "Too many arguments")
        ]

        with patch('sys.stdout', new=StringIO()) as fake_out:
            for inp, out in test_cases:
                self.client.do_attack(inp)
                self.client.s.sendall.assert_not_called()
