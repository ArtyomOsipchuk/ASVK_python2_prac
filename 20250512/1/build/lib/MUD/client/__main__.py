...
class CowNetcat(cmd.Cmd):
    ...
    def do_documentation(self, arg):
        ...
        try:
            docs_path = files("MUD.docs") / 'html' / "index.html"
...
def run():
    ...
    parser.add_argument(
        "host", nargs="?", default="localhost", help="Адрес хост-сервера"
    )
    parser.add_argument(
        "port", nargs="?", type=int, default=1337, help="Порт хост-сервера"
    )
    parser.add_argument(
        "--file", default=None, help="Получение команд из командного файла"
    )
